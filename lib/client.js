...
function Client(options, request, wrap, when, all) {
  assert(options.version, 'You must specify a version when connecting (e.g. `connect({version: 3})`)');
  this._version = 'application/vnd.github.' + (typeof options.version === 'number' && 'v') + options.version + '+json';
  this._request = request;
  this._wrap = wrap;
  this._when = when;
  this._all = all;
  this._cache = options.cache;
  this._oneTimePassword = false;
  if (options.auth) {
    if (typeof options.auth === 'string') {
      ...
      if (options.auth.otp) {
        this._oneTimePassword = options.auth.otp;
      }
    } else {
...
Client.prototype.requestBufferWithHeaders = function (method, path, query) {
  return this._wrap(function () {
    ...
    var headers = {};
    if (!IS_BROWSER) {
      headers['user-agent'] = 'scriptit/github-basic';
    }
